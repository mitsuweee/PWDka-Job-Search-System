...
const registerUser = async (req, res, next) => {
    ...
    if(!id || !first_name || !middle_inital || !last_name || !address || !city || !gender|| !birth_date || !email || !password || !contact_number || !disability_id || !formal_picture || !picture_with_id || !picture_of_pwd_id){
                    ...
                    if(emailRows.length > 0){
                        ...
                        if(emailRows.length > 0){
                            ...
                            if(emailRows.length > 0){
                                ...
                                const disabilityRows = await connection.query(selectDisabilityIdQuery, [disability_id])
                                ...
                                if(disabilityRows.length === 0){
                                    ...
                                    const values = userModel(id, first_name, middle_inital, last_name, address, city, gender, birth_date, email, hashedPassword, contact_number, disability_id, formal_picture, picture_with_id, picture_of_pwd_id)
                                    const userObj = [values.id, values.first_name, values.middle_initial, values.last_name, values.address, values.city, values.gender, values.birth_date, values.email, values.password, values.contact_number, values.disability_id, values.formal_picture, values.picture_with_id, values.picture_of_pwd_id]
        
                                    await connection.query(insertQuery, userObj)
                                    ...
                                    return res.status(200).json({
                                        successful : true, 
                                        message : "Successfully Registered User! Before having the capability to access our website, We will check if all the data you have inputed is legitimate. Please wait for our email verification if the User is accepted or rejected. Thank you!"
                                    })
                                }
                            }
                        }
    ...
    if (!email || !password) {
        ...
        try {
            ...
            try {
                ...
                const userRows = await connection.query(selectUserQuery, [email])
                ...
                if (!passwordMatch) {
                    ...
                    return res.status(400).json({
                        successful: false,
                        message: "The User's Account is under Verification. Please wait for the email confirmation."
                    })
...
const updateUser = async (req,res, next) => {
    ...
    if(id == null || address == null || city == null || contactNumber == null || formalPicture == null){
        return res.status(404).json({
            successful : false,
            message : "One or more details are missing"
        })
        ...
        try{
            ...
            try{
                ...
                const values = [address, city, contactNumber, formalPicture, id]
...
//     // Convert values to lowercase where applicable
//     const lowercasedValues = {
//         first_name: first_name.toLowerCase(),
//         middle_initial: middle_initial.toLowerCase(),
...
//     // Validation checks
//     if (!id || !lowercasedValues.first_name || !lowercasedValues.middle_initial || !lowercasedValues.last_name || !lowercasedValues.address || !lowercasedValues.city || !lowercasedValues.gender || !birth_date || !lowercasedValues.email || !password || !contact_number || !disability_id || !formal_picture || !picture_with_id || !picture_of_pwd_id) {
//         return res.status(404).json({
//             successful: false,
//             message: "One or more details are missing"
//         });
...
//             message: "Invalid ID number"
//         });
//     } else if (!util.checkCharacters(lowercasedValues.first_name)) {
//         return res.status(400).json({
//             successful: false,
//             message: "First Name Must only contain alphabetical characters"
//         });
//     } else if (!util.checkMiddleInitial(lowercasedValues.middle_initial)) {
//         return res.status(400).json({
//             successful: false,
//             message: "Middle Initial should be a single character"
//         });
//     } else if (!util.checkCharacters(lowercasedValues.last_name)) {
//         return res.status(400).json({
//             successful: false,
//             message: "Last Name Must only contain alphabetical characters"
//         });
//     } else if (util.checkSpecialChar(lowercasedValues.address)) {
//         return res.status(400).json({
//             successful: false,
//             message: "Special Characters are not allowed in Address"
//         });
//     } else if (!util.checkCharacters(lowercasedValues.city) || !lowercasedValues.city.endsWith("city")) {
//         return res.status(400).json({
//             successful: false,
//             message: "City Must only contain alphabetical characters"
//         });
//     } else if (!util.checkCharacters(lowercasedValues.gender)) {
//         return res.status(400).json({
//             successful: false,
...
//             message: "User must be 18 years old or higher"
//         });
//     } else if (!util.checkEmail(lowercasedValues.email)) {
//         return res.status(400).json({
//             successful: false,
//             message: "Invalid Email"
//         });
...
//         return res.status(400).json({
//             successful: false,
//             message: "Invalid Password Format. It should have at least one digit, one uppercase, one lowercase, one special character, and at least 8 in length"
//         });
//     } else if (password !== confirmPassword) {
//         return res.status(400).json({
//             successful: false,
...
//                 } else {
//                     const selectEmailQuery = `SELECT email from user WHERE email = ?`;
//                     const emailRows = await connection.query(selectEmailQuery, [lowercasedValues.email]);
...
//                         });
//                     } else {
//                         const selectEmailQueryAdmin = `SELECT email from admin WHERE email = ?`;
//                         const emailRowsAdmin = await connection.query(selectEmailQueryAdmin, [lowercasedValues.email]);

//                         if (emailRowsAdmin.length > 0) {
//                             return res.status(400).json({
//                                 successful: false,
//                                 message: "Email already exists"
//                             });
//                         } else {
//                             const selectEmailQueryCompany = `SELECT email from company WHERE email = ?`;
//                             const emailRowsCompany = await connection.query(selectEmailQueryCompany, [lowercasedValues.email]);

//                             if (emailRowsCompany.length > 0) {
//                                 return res.status(400).json({
//                                     successful: false,
//                                     message: "Email already exists"
//                                 });
...
//                                     const insertQuery = `INSERT into user (id, first_name, middle_initial, last_name, address, city, gender, birth_date, email, password, contact_number, disability_id, formal_picture, picture_with_id, picture_of_pwd_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
//                                     const hashedPassword = await bcrypt.hash(password, 10);
//                                     const values = userModel(id, lowercasedValues.first_name, lowercasedValues.middle_initial, lowercasedValues.last_name, lowercasedValues.address, lowercasedValues.city, lowercasedValues.gender, birth_date, lowercasedValues.email, hashedPassword, contact_number, disability_id, formal_picture, picture_with_id, picture_of_pwd_id);
//                                     const userObj = [values.id, values.first_name, values.middle_initial, values.last_name, values.address, values.city, values.gender, values.birth_date, values.email, values.password, values.contact_number, values.disability_id, values.formal_picture, values.picture_with_id, values.picture_of_pwd_id];
...
//                                     const mailOptions = {
//                                         from: 'livcenteno24@gmail.com',
//                                         to: lowercasedValues.email,
//                                         subject: 'Account Verification',
//                                         text: `Dear ${lowercasedValues.first_name},\n\nThank you for registering. Your account is under review and will be activated once verified. We will notify you once the verification process is complete.\n\nBest regards,\nPWDKA Team`
//                                     };
